...
//		System.out.println(dt);
	...
	private static void testC()
	{
		...
		int a=3, b=-1;
		System.out.println("Test C0:"+(a+b));
		Point[] points={
				new Point(2344, 100),
				new Point(3654, 104),
				new Point(3952, 95),
...
//				new Point(2, 4),
		...
		Point[] points1=generatePointsForTestCaseB();
		
		DT dt=new DataStructure(), dt1=new DataStructure();
		for (Point point : points1)
			dt.addPoint(point);
		for (Point point : points)
			dt1.addPoint(point);
		Point[] result=dt1.getPointsInRangeOppAxis(108, 259, false);
//		System.out.println(dt1);
		Point[] closestPoints;
		System.out.println("Median: "+dt.getMedian(true));
		for (int width=2; width<=1000; width++)
		{
			closestPoints=dt.nearestPairInStrip(dt.getMedian(true), width, true);
			System.out.println(Arrays.toString(closestPoints)+", distance: "+
			                   DataStructure.findDistance(closestPoints[0], closestPoints[1])+", width: "+width);
		}
		closestPoints=dt.nearestPair();
		System.out.println(Arrays.toString(closestPoints)+", distance: "+
		                 DataStructure.findDistance(closestPoints[0], closestPoints[1]));
		
//		dt1.narrowRange(2, 8, true);
//		System.out.println(dt1);
	...
	private static void testExpectedPoints(String testName, Point[] expectedPoints, Point[] actualPoints)
	{
		if (!equalAsLists(actualPoints, expectedPoints))
		{
			if (equalAsSets(actualPoints, expectedPoints))
				System.out.println(
						"Test "+testName+": order wrong!\n expected="+Arrays.toString(expectedPoints)+",\n actual=  "+
						Arrays.toString(actualPoints));
			else
				System.out.println(
						"Test "+testName+": wrong!\n expected="+Arrays.toString(expectedPoints)+",\n actual=  "+
						Arrays.toString(actualPoints));
	...
	private static boolean equalAsSets(Point[] actualPoints, Point[] expectedPoints)
	{
		...
		for (Point expectedPoint : expectedPoints)
		{
			...
			{
				if (expectedPoint.equals(actualPoint)) exists=true;
			}
