...
public class Assignment2Test
{
	...
	private int maxChecksPerTest=1; //How many randomized options per a field of points to test
	private int maxDifferentTests=1000000; //how many different fields of points to test
...
//		test.saveTestToCode();
	...
	public void setMaxChecksPerTest(int num)
	{
		maxChecksPerTest=num;
	}
	...
	{
		int k=1000;
		for (int i=0; /*i<maxDifferentTests*/; i++)
		{
			if (i/k>0)
			{
			...
			foundError=!testSorting()
			           || !testGetDensity()
			           || !testGetLargestAxis()
			           || !testGetMedian(true)
			           || !testGetMedian(false)
			           || !testNearestPair();
			for (int j=0; j<maxChecksPerTest; j++)
			{
	...
	{
		testLogger.setLevel(state ? Level.ALL : Level.WARNING);
	}
	
	public void autoTestWithValues()
	{
	...
	public final void initializeNewTest()
	{
		...
		HashSet Xnums=new HashSet();
		HashSet Ynums=new HashSet();
		Random rand=new Random();
		size=rand.nextInt(maxPoints-3)+3;
		...
		while (i<size)
		{
			...
			while (Xnums.contains(x))
			{
				x=rand.nextInt(maxPoints*10);
			}
			while (Ynums.contains(y))
			{
				y=rand.nextInt(maxPoints*10);
			}
			p=new Point(x, y);
	...
	public final void savePointsToFile()
	{
		...
		try
		{
			...
			FileOutputStream fileOut=new FileOutputStream(pointsFile);
			ObjectOutputStream out=new ObjectOutputStream(fileOut);
			out.writeInt(size);
			for (int i=0; i<size; i++)
			{
				System.out.println("("+arraySortedX[i].getX()+", "+arraySortedX[i].getY()+")");
	...
	{
		return checkEquality(arraySortedX, dt.getPointsInRangeRegAxis(Integer.MIN_VALUE+1, Integer.MAX_VALUE-1, true))
		       &&
		       checkEquality(arraySortedY, dt.getPointsInRangeRegAxis(Integer.MIN_VALUE+1, Integer.MAX_VALUE-1, false));
		
	}
	
	
	public boolean testGetPointsInRangeRegAxisX(int max, int min)
	{
	...
	public boolean testGetPointsInRangeRegAxisY(int max, int min)
	{
	...
	public boolean testGetMedian(boolean axis)
	{
		...
		{
			testLogger.severe("Failed GetMedian axis:"+axis+", got: "+result.toString()+" expected: "+
			                  expected.toString());
			return false;
		}
		else
	...
	private Point[] closestPair(Point[] arr)
	{
		...
		double minDistance=Integer.MAX_VALUE, newDist=0;
